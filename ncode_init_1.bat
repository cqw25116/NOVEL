...
::１作品のみ発表 1016
::http://syosetu.com/searchuser/search/?&order=novelcnt&minnovel=1&maxnovel=1&p=1
::set max=25
set max=765
set save_html=.\html\yomou\
set save_csv=.\yomou\
type nul > s_user.csv
::for /L %%a in (1,1,%max%) do (
   curl "http://syosetu.com/searchuser/search/?&order=novelcnt&minnovel=2&p=[1-%max%]" -o "%save_html%_user_#1.html"
 ::   wget "http://syosetu.com/searchuser/search/?&order=novelcnt&minnovel=2&p=%%a" -nv -O -| nkf>%save_html%s_user_%%a.html
    
::)

goto :end
::    wget "http://syosetu.com/searchuser/search/?&order=novelcnt&minnovel=2&p=%%a" -nv -O -| nkf>%save_html%s_user_%%a.html
::|sed -e "s/<[^>]*>//g"  |grep -A 4 "ID：" |sed -e "s/^ID：//g"  -e "s/^小説投稿数：//g"  -e "s/^文字数：//g" -e "s/,//g" -e "s/^〔//g" -e "s/〕$//g" -e "s/\,/，/g" |grep -v "^--" | nkf -Lu |paste -d, - - - - - | nkf -Lw > %save_csv%s_user_%%a.csv
 
::     tee %save_csv%s_user_%%a.csv>>s_user.csv

goto :end
::?word=%2C
::http://syosetu.com/searchuser/search/?&order=novelcnt&minnovel=2&p=1
